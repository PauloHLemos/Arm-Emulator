...
bool is branch(char *opcode);

uint32_t translate_instruction(char *instruction, ST_Node *st_head_ptr, uint32_t current_address) {
	...
	if (is_multiply(opcode)) {
		...
		instruction_struct = translate_branch(instruction, st_head_ptr);
	} else {
