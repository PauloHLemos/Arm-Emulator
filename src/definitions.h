...
struct Pipeline {
	...
	uint32_t (*fetch)(struct State *state_struct_ptr, struct Pipeline *pipeline);
	uint32_t instruction_binary;
	struct Instruction (*decode)(uint32_t instruction_binary, struct Pipeline *pipeline);
	void (*execute)(struct Instruction instruction, struct State *state, struct Pipeline *pipeline);
};
