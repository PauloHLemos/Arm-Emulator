...
bool condition_met(struct Instruction *instruction_ptr, struct State *state_ptr) {
	bool N = get_CPSR_bit(state_ptr, 31);
	bool Z = get_CPSR_bit(state_ptr, 30);
	bool V = get_CPSR_bit(state_ptr, 28);
...
bool execute(struct Instruction *instruction_ptr, struct State *state_ptr) {
	...
	switch(instruction_ptr->type) {
		case DATA_PROCESSING:	   data_processing(state_ptr, instruction_ptr); break;
		case MULTIPLY:		   multiply(state_ptr, instruction_ptr); break;
		case SINGLE_DATA_TRANSFER: single_data_transfer(state_ptr, instruction_ptr); break;
		case BRANCH:		   branch(state_ptr, instruction_ptr); return true;
	}
	return false;
}
